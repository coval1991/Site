...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const eligibleHolders = await User.countDocuments({ isEligibleForDividends: true });
...
router.get('/snapshot', async (req, res) => {
  try {
    ...
    const combinedHolders = dbHolders.map(dbHolder => {
      const blockchainHolder = blockchainHolders.find(bh => 
        bh.address.toLowerCase() === dbHolder.walletAddress.toLowerCase()
      );

      return {
        walletAddress: dbHolder.walletAddress,
        cfdBalance: blockchainHolder ? blockchainHolder.balance : dbHolder.cfdBalance,
        isEligibleForDividends: blockchainHolder ? blockchainHolder.eligible : dbHolder.isEligibleForDividends,
        firstTokenPurchase: blockchainHolder ? blockchainHolder.firstPurchase : dbHolder.firstTokenPurchase,
    ...
    blockchainHolders.forEach(bh => {
      const exists = combinedHolders.find(ch => 
        ch.walletAddress.toLowerCase() === bh.address.toLowerCase()
      );
...
router.get('/ico-stats', async (req, res) => {
  try {
    ...
    const stats = await Promise.all(phases.map(async (phase) => {
